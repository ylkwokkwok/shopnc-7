...
function update_data(id) {//更新
    var get_text = $.ajax({
        type: "POST",
        url: 'index.php?act=web_api&op=code_update&action=org',
    ...
    for (var i = 1; i <= recommend_max; i++) {//防止数组下标重复
        if ($(".middle dl[recommend_id='"+i+"']").size()==0) {//编号不存在时添加
            ...
            $("#add_recommend_list").before('<dl select_recommend_id="'+i+'"><dt><h4 class="dialog-handle-title">商品推荐模块标题名称</h4>'+
                '<div class="dialog-handle-box"><span class="left"><input name="recommend_list_org['+i+'][recommend][name]" value="商品推荐" type="text" class="w200"></span>'+
...
function select_recommend_goods(goods_id) {//商品选择
    ...
    text_append += '</div>';
    text_append += '<input name="recommend_list_org['+id+'][goods_list]['+goods_id+'][goods_id]" value="'+goods_id+'" type="hidden">';
    text_append += '<input name="recommend_list_org['+id+'][goods_list]['+goods_id+'][market_price]" value="'+market_price+'" type="hidden">';
    text_append += '<input name="recommend_list_org['+id+'][goods_list]['+goods_id+'][goods_name]" value="'+goods_name+'" type="hidden">';
    text_append += '<input name="recommend_list_org['+id+'][goods_list]['+goods_id+'][goods_price]" value="'+goods_price+'" type="hidden">';
    text_append += '<input name="recommend_list_org['+id+'][goods_list]['+goods_id+'][goods_pic]" value="'+replace_url(goods_pic)+'" type="hidden">';
