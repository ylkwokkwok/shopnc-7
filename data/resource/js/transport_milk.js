$(function(){
	...
	TransTpl += "<i class=\"icon-map-marker\"></i>为指定地区设置运费<\/a>\n";
	...
	SpecialMessage = '';
	SpecialMessage += "<span  error_type=\"area\" class=\"msg J_Message\" style=\"display:none\"><i class=\"icon-exclamation-sign\"><\/i>指定地区为空或指定错误<\/span>\n";
...
/*	选择完区域后，确定事件*/
	$('#dialog_areas').on('click','.J_Submit',function (){
		...
		//记录弹出层内所有已被选择的checkbox的值，记录到CityValue，CityText中
		$('#J_CityList').find('input[type="checkbox"]').each(function(){
			if ($(this).attr('checked')){
				CityValue += $(this).val()+',';
				CityText += $(this).next().html()+',';
			}
		});
