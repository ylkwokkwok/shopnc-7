...
class tohome_store_billControl extends BaseSellerControl {
    /**
     * 每次导出多少条记录
     * @var unknown
     */
	...
	$condition .='  AND `order`.wx_pay_sn != "") ';
        ...
        $count = $model_self_bill->getTohomeCount($condition);
        $resultlist = $model_self_bill->totaltohomeOrder($condition,$count[0]['orderNum']);
        Tpl::output('resultlist',$resultlist);
        Tpl::output('show_page',$model_self_bill->showpage());
        $this->profile_menu('resultlist','resultlist');
        Tpl::showpage('tohome_store_bill_total');
    }
...
//        }
	...
	$condition .=') || (`order`.wx_pay_sn != "") ';
		...
		if (!is_numeric($_GET['curpage'])){
			$count = $model_self_bill->getSallTohomeOrderCount($condition);
			$array = array();
			if ($count > self::EXPORT_SIZE ){	//显示下载链接
				...
				Tpl::output('murl','index.php?act=tohome_store_bill&op=index');
				Tpl::showpage('export.excel');
			}else{	//如果数量小，直接下载
				$data = $model_self_bill->getSallTohomeOrderList($condition,self::EXPORT_SIZE);
				$this->createExcel($data);
			}
		}else{	//下载
			$limit1 = ($_GET['curpage']-1) * self::EXPORT_SIZE;
			$limit2 = self::EXPORT_SIZE;
			$data = $model_self_bill->getSallTohomeOrderList($condition,"{$limit1},{$limit2}");
			$this->createExcel($data);
		}
	}
	...
	private function createExcel($data = array()){
		...
		$excel_obj->addWorksheet($excel_obj->charset('到户销售情况',CHARSET));
		$excel_obj->generateXML($excel_obj->charset('到户销售情况',CHARSET).'-'.date('Y-m-d-H',time()));
	}
}
