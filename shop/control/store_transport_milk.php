...
class store_transport_milkControl extends BaseSellerControl {
	...
	public function listOp(){
		...
		Tpl::showpage('store_transport_milk.list');
	}
	...
	public function addOp(){
		...
		Tpl::output('extend',$extend);
		$areas = $model_transport->getReceive();
		...
		self::profile_menu('transport','transport');
		Tpl::showpage('store_transport_milk.add');
	}
	...
	public function saveOp(){
		...
		if ($result){
			header('location: index.php?act=store_transport_milk&type='.$_POST['type']);exit;
		}else{
			showMessage(Language::get('transport_op_fail'),$_SERVER['HTTP_REFERER'],'html','error');
		}
	}
    ...
    public function offpay_areaOp()
    {
        ...
        Tpl::showpage('store_transport_milk.offpay_area');
    }
	...
	private function profile_menu($menu_type,$menu_key='') {
		...
		switch ($menu_type) {
            ...
            case 'offpay_area':
				$menu_array = array(
				1=>array('menu_key'=>'transport',	'menu_name'=>Language::get('nc_member_path_postage'),		'menu_url'=>'index.php?act=store_transport_milk'),
				);
                //if (checkPlatformStore()) {
                /* lyq@newland 修改开始 **/
                /* 时间：2015/06/08     **/
