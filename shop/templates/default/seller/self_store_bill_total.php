...
<form method="get" action="index.php" target="_self">
  <table class="search-form">
    <input type="hidden" name="act" value="self_store_bill" />
    <input type="hidden" name="op" value="index" />
    <tr>
       <th>自取点：</th>
        <td class="w160"><input type="text" class="text w150" name="self_cd" value="<?php echo $_GET['self_cd']; ?>" /></td>
  ...
  <tbody>
     <?php if (!empty($output['resultlist']) && is_array($output['resultlist'])) { ?>
      <?php foreach($output['resultlist'] as $bill_info) { ?>
         <tr>
               <!--<td></td>-->
               <td><?php echo $bill_info['self_cd'];?></td>
